...
void system_render_update(ECS* ecs, RenderContext* render_context, Entity camera)
{
	...
	Transform* camera_transform = ecs_get_component(ecs, camera, COMP_TRANSFORM);

	for (size_t i = 0; i < sprite_array.num_components; ++i)
	{
		const Entity entity = sprite_array.index_to_entity[i];
		const ComponentSignature sig = entity_signature(&ecs->entity_data, entity);
		const Transform* transform = ecs_get_component(ecs, entity, COMP_TRANSFORM);
		const Sprite* sprite = (Sprite*)(sprite_array.array) + i;
		Rect frame = {0, 0, sprite->texture.width, sprite->texture.height };
		if (sig & COMP_SIG(COMP_ANIMATION))
		{
			const Animation* animation =
				ecs_get_component(ecs, entity, COMP_ANIMATION);
