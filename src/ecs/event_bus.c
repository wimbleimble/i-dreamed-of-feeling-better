...
void event_bus_init(EventBus* bus)
{
	for(size_t type = 0; type < SYS_NUM; ++type)
		bus->buffers[type].head = bus->buffers[type].tail = bus->buffers[type].buffer =
			calloc(BUS_CAPACITY, bus->event_size);
}

static inline void event_buffer_push(EventBuffer* buffer, void* event, size_t event_size)
{
...
{
	// TODO bounds checking and loop around
	for (; buffer->tail != buffer->head; buffer->tail = (uint8_t*)buffer->tail + 1)
...
{
	for (SystemType type = 0; type < SYS_NUM; ++type)
		event_buffer_dispatch(&bus->buffers[type], EVENT_HANDLERS[type], ecs);
}